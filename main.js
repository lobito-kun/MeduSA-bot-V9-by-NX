...
global.reloadHandler = function () {
  ...
  conn.bye = 'Adios @user!'
  conn.spromote = 'A @user le dieron admin!'
  conn.sdemote = 'A @user le sacaron admin!'
...
global.reload = (_event, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (dir in require.cache) {
      ...
      if (fs.existsSync(dir)) conn.logger.info(`RE - ACTIVANDO NUEVO PLUGIN '${filename}'`)
      else {
        conn.logger.warn(`PLUGIN ELIMIADO: '${filename}'`)
        return delete global.plugins[filename]
      }
    } else conn.logger.info(`ACTIVANDO NUEVO PLUGIN '${filename}'`)
    let err = syntaxerror(fs.readFileSync(dir), filename)
    if (err) conn.logger.error(`syntax CUANDO SE CARGABA EL PLUGIN: '${filename}'\n${err}`)
    else try {
