...
${sortedLim.slice(0, len).map(({ jid, limit }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${limit} Leaf*`).join`\n`}
...
handler.help = ['leaderboard [numero de usuarios]', 'lb [Numero de usuarios]']
handler.tags = ['xp']
