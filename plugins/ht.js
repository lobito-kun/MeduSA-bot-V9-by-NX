let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'Alguien todavia esta haciendo el texto del trono en este chat... Espera a que termine'
  else conn.hartatahta[m.chat] = true
  m.reply('_Haciendo..._\n*Espera 2 minutos!*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'tesoro Tahta.png', '*© MeduS.A_bot-v2*\nMade with FFmpeg', m)
  } finally {
    delete conn.hartatahta[m.chat]
  }
}
handler.help = ['trono <texto>']
handler.tags = ['escribir']
handler.command = /^((harta)?trono)$/i
handler.limit = true
