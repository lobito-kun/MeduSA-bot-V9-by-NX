...
let handler = async (m, { conn, args }) => {
  ...
  let none = '_None_'
  let caption = `
Contenido${isFamilyFriendly ? ' ' : ' *NO* '}Family Friendly
Posteado por ${name} (${authorname || ''}) (${authorlink || ''})
Posteado en ${new Date(uploadDate).toLocaleDateString('id', dateConfig)}
Size: ${contentSize || unknown}
Duracion: ${clockString(+ new Date(duration))}
Genero: ${genre || none}
Calida: ${videoQuality ? videoQuality : unknown}
