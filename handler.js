...
module.exports = {
  async handler(chatUpdate) {
    ...
    try {
      ...
      let issMod = isOwner || global.mods.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
		  ...
		  if (plugin.smod && !issMod) { // S Moderator
            fail('smod', m, this)
            continue
          }
          ...
          if (xp > 1) m.reply('que -_- ') // Hehehe
          else m.exp += xp
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `💰 *MeduS.A-bank*\n\nSe han acabado tus leaf, para obtener más Leaf utiliza *${usedPrefix}buy 5  y cambia tu XP por Leaf!*`, m)
          ...
          try {
            ...
            if (m.limit) m.reply(+ m.limit + ' Gastaste 5 leaf, para ver tu dinero escribe !leaf')
          }
          break
        }
      }
  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
        ...
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Bienvenido, @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc) :
      ...
      case 'demote':
        if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```No es mas admin```')
...
`.trim(), m.message, {
  ...
  async onCall(json) {
    ...
    await this.sendMessage(from, 'MeduS.A-bot | *AVISO\n\nLlamar al bot = baneo permanente + bloqueo.', MessageType.extendedText)
